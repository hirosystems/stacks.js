...
Transaction signing allows users execute Clarity smart contracts that are relevant to your app and then handle the result as appropriate.
...
Users must authenticate to an app before the `connect` package will work to prompt them to sign and broadcast transactions to the Stacks blockchain with an authenticator such as [the Stacks Wallet](https://www.hiro.so/wallet/install-web).
