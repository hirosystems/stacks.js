...
export function runNetworkTests() {
  test('prices-v1', (t) => {
    ...
    mynet.getNamePrice('test.id')
      .then(response => 
        t.deepEqual(response, { units: 'BTC', amount: bigi.fromByteArrayUnsigned('5500') }))

    mynet.getNamespacePrice('id')
      .then(response => 
        t.deepEqual(response, { units: 'BTC', amount: bigi.fromByteArrayUnsigned('650000') }))
  })
  ...
  test('prices-v2', (t) => {
    ...
    mynet.getNamePrice('test-tokens.id')
      .then(response => 
        t.deepEqual(response, { units: 'STACKS', amount: bigi.fromByteArrayUnsigned('1733000') }))
  ...
  test('accounts', (t) => {
    ...
    mynet.getAccountBalance(addr, 'STACKS')
      .then(response => 
        t.deepEqual(response, bigi.fromByteArrayUnsigned('69129')))
    mynet.getAccountBalance(addr2, 'NOPE')
      .then(response => 
        t.deepEqual(response, bigi.fromByteArrayUnsigned('0')))
