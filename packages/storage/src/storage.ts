...
export class Storage {
  ...
  async getFile(
    ...
    const storedContents = await this.getFileContents(path, opt.app, opt.username, 
    ...
    if (storedContents === null) {
      ...
      return this.handleSignedEncryptedContents(path, storedContents,
  ...
  async getFileSignedUnencrypted(path: string, opt: GetFileOptions) {
    ...
    try {
      const [fileContents, signatureContents, gaiaAddress] = await Promise.all([
        ...
        this.getFileContents(sigPath, opt.app, opt.username,
