...
test('deleteFile', async () => {
  ...
  await storage.deleteFile(path, options)
    .then(success)
...
test('deleteFile throw on 404', async () => {
  ...
  await storage.deleteFile(path, { wasSigned: false })
...
test('deleteFile removes etag from map', async () => {
  ...
  const uploadToGaiaHub = jest.fn().mockResolvedValue(JSON.stringify({publicURL: 'url', etag: testEtag }))
  const deleteFromGaiaHub = jest.fn()
  jest.mock('../src/hub', () => ({
    uploadToGaiaHub,
    deleteFromGaiaHub
...
test('getFile unencrypted, unsigned - multi-reader', async () => {
  ...
  fetchMock.once(nameRecordContent)
  ...
  await storage.getFile(path, optionsNameLookupUrl)
    .then((file) => {
      expect(file).toBeTruthy()
      expect(JSON.parse(<string>file)).toEqual(JSON.parse(fileContents))
    })
  ...
  await storage.getFile(path, optionsNoApp)
    .then((file) => {
      expect(file).toBeTruthy()
      expect(JSON.parse(<string>file)).toEqual(JSON.parse(fileContents))
    })
...
test('putFile unencrypted, not signed', async () => {
  ...
  await storage.putFile(path, fileContent as any, options)
...
test('putFile & getFile encrypted, not signed', async () => {
  ...
  const decryptOptions = { decrypt: true }
  // put and encrypt the file
  await storage.putFile(path, fileContent, encryptOptions)
    .then((publicURL: string) => {
      expect(publicURL).toBeTruthy()
    }).then(() => {
...
test('putFile & getFile encrypted, signed', async () => {
  const privateKey = 'a5c61c6ca7b3e7e55edee68566aeab22e4da26baa285c7bd10e8d2218aa3b229'
  const appConfig = new AppConfig(['store_write'], 'http://localhost:3000')
  const userSession = new UserSession({ appConfig })
  ...
  const uploadToGaiaHub = jest.fn().mockResolvedValueOnce({
    publicURL: fullReadUrl
  ...
  // put and encrypt the file
  return storage.putFile('doesnt-matter.json', fileContent, encryptOptions)
    .then((publicURL: string) => {
      expect(publicURL).toEqual(fullReadUrl)

      const putFileContents = uploadToGaiaHub.mock.calls[0][1]
      const contentsObj = JSON.parse(putFileContents)
      fetchMock.once(putFileContents)
    ...
    }).then(() => storage.getFile('file.json', decryptOptions).then((readContent: string) => {
      expect(readContent).toEqual(fileContent)
    }))
    .then(() => storage.getFile('file.json', {
      decrypt: true,
      verify: true,
    ...
    .then(() => storage.getFile('badPK.json', decryptOptions)
    ...
    .then(() => storage.getFile('badPK.json', {
      decrypt: true,
      verify: true,
      username: 'applejacks.id',
      app: 'origin'
    ...
    .then(() => storage.getFile('badSig.json', decryptOptions)
      .then(() => fail('Should not successfully decrypt file'))
      .catch((err: Error) => {
...
test('putFile & getFile unencrypted, signed', async () => {
  ...
  const getFullReadUrl = jest.fn().mockImplementation((path) => 
    Promise.resolve(pathToReadUrl(path)))
...
test('base64 output size calculation', async () => {
  const testLengths = [0, 1, 2, 3, 4, 8, 100, 500, 1000]
  for (let i = 0; i < 10; i++) {
    testLengths.push(Math.floor(Math.random() * Math.floor(1030)))
  }
...
test('promises reject', async () => {
  ...
  await storage.putFile(path, 'hello world', { encrypt: false })
...
test('getFileUrl', async () => { 
  ...
  await storage.getFileUrl('foo.json', {})
    .then(x => {
      expect(x).toEqual('https://gaia.testblockstack.org/hub/19MoWG8u88L6t766j7Vne21Mg4wHsCQ7vk/foo.json')
    })
})
...
test('getFile throw on 404', async () => {
  ...
  await storage.getFile('foo.json', optionsNoDecrypt)
  ...
  await storage.getFile('foo.json', optionsDecrypt)
...
test('connectToGaiaHub with an association token', async () => {
  ...
  expect(verified).toBe(true)
  expect(hubServer).toEqual((decodeToken(jsonTokenPart).payload as any).hubUrl)
  expect(gaiaAssociationToken).toEqual((decodeToken(jsonTokenPart).payload as any).associationToken)  
})
...
test('getUserAppFileUrl', async () => {
  ...
  await storage.getUserAppFileUrl(path, name, appOrigin)
    .then((url: string) => {
...
test('listFiles gets a new gaia config and tries again', async () => {
  ...
  const files: string[] = []
  await storage.listFiles((name: string) => {
    files.push(name)
    return true
  }).then((count: number) => {
    expect(files.length).toEqual(1)
