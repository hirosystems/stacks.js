...
test('Estimate transaction transfer fee', async () => {
  ...
  expect(fetchMock.mock.calls[0][0]).toEqual(getTransactionFeeEstimateApiUrl(mainnet));
  ...
  expect(fetchMock.mock.calls[1][0]).toEqual(getTransactionFeeEstimateApiUrl(testnet));
...
test('Make STX token transfer with fetch account nonce', async () => {
  ...
  const apiUrl = getAccountApiUrl(network, senderAddress);
...
test('Make sponsored STX token transfer with sponsor fee estimate', async () => {
  ...
  expect(fetchMock.mock.calls.length).toEqual(1);
  expect(fetchMock.mock.calls[0][0]).toEqual(getTransactionFeeEstimateApiUrl(network));
...
test('Make sponsored contract call with sponsor nonce fetch', async () => {
  ...
  expect(fetchMock.mock.calls.length).toEqual(1);
  expect(fetchMock.mock.calls[0][0]).toEqual(getAccountApiUrl(network, sponsorAddress));
...
test('Transaction broadcast success', async () => {
  ...
  expect(fetchMock.mock.calls.length).toEqual(1);
  expect(fetchMock.mock.calls[0][0]).toEqual(getBroadcastApiUrl(network));
...
test('Make contract-call with network ABI validation', async () => {
  ...
  expect(fetchMock.mock.calls.length).toEqual(1);
  expect(fetchMock.mock.calls[0][0]).toEqual(getAbiApiUrl(network, contractAddress, contractName));
});
...
test('Make contract-call with network ABI validation failure', async () => {
  ...
  const abiUrl = getAbiApiUrl(network, contractAddress, contractName);
...
test('Call read-only function', async () => {
  ...
  const apiUrl = getReadOnlyFunctionCallApiUrl(
    network,
    contractAddress,
    contractName,
    functionName
