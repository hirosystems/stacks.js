...
function putFileImpl(caller, path, content, options) {
    ...
    if (!opt.encrypt && opt.sign) {
        var signatureObject = ec_1.signECDSA(privateKey, content);
        var signatureContent_1 = JSON.stringify(signatureObject);
        return hub_1.getOrSetLocalGaiaHubConnection(caller)
    ...
    if (opt.encrypt && !opt.sign) {
        ...
        var signatureObject = ec_1.signECDSA(privateKey, cipherText);
